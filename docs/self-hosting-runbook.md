...
2026-09-05 状态：root/www A 记录已生效，证书已签发（有效期至 2026-12-03），HTTPS 配置和 HTTP/www 重定向已启用。续期 dry-run（含 Nginx reload hook）成功。腾讯云轻量实例防火墙放通后，公网 HTTPS 强制解析到 `81.71.71.252` 验收通过：主页、学生登录、企业登录、后台登录均返回 200，`www` 返回 308 到主域名。当前维护命令保留如下。

DNSPod 的 `@` 和 `www` 添加 A 记录指向服务器，线路默认，TTL 默认。保留已有 MX/TXT 记录，同名冲突先核对，不直接删除。

官方说明：https://docs.dnspod.cn/dns/help-a/

正式切换前已再次核对旧站自导出快照后的新增业务数据和文件；除一次性导出防重放标记外，无业务差异。当前导入不是双向同步系统。正式域名上线后，应引导用户使用 `https://star-plan.com`，避免旧站继续产生新数据。
