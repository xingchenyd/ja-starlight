...
const fallbackSlides:Slide[]=[
 {image:"/media/ja-career-fair.jpg",eyebrow:"成长活动",title:"湖南未来职业市集：把职业探索带到真实现场",meta:"青年与企业面对面 · 活动报名",href:"/workspace?role=student&tab=activities"},
...
export default function HomeCarousel(){
 ...
 useEffect(()=>{let live=true;fetch("/api/catalog").then(response=>response.json()).then(data=>{if(!live)return;const published=(data.records||[]).filter((record:CatalogRecord)=>record.kind==="activity"||record.kind==="content").sort((a:CatalogRecord,b:CatalogRecord)=>Number(b.payload.featured)-Number(a.payload.featured)||Number(b.payload.sortOrder||0)-Number(a.payload.sortOrder||0)).map(fromCatalog).filter(Boolean) as Slide[];if(published.length)setSlides([...published.slice(0,4),...fallbackSlides].slice(0,6))}).catch(()=>{});return()=>{live=false}},[]);
 ...
 return <section className="story-stage shell" aria-label="JA 活动与成长内容轮播" onMouseEnter={()=>setPaused(true)} onMouseLeave={()=>setPaused(false)}>
  <div className="story-heading"><div><p className="eyebrow">LIVE STORIES · STAR PLAN</p><h2>正在发生的成长故事</h2></div><p>企业与 JA 审核发布的封面会原样出现在这里。推荐内容优先展示，左右切换，点击即可进入报名或阅读。</p></div>
