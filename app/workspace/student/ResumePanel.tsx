...
export default function ResumePanel({ profile, upload, save, flash }: {
  ...
  return <>
    ...
    <SidePanel open={open && Boolean(pending)} title="确认简历识别结果" description="自动解析服务尚未接入时会保留原文件，并允许你手动确认和修正关键信息。" dirty={Boolean(pending)} onClose={() => { setOpen(false); setPending(null); }} footer={<button className="primary-btn" disabled={busy} onClick={confirm}>{busy ? "正在保存…" : "确认并更新个人资料"}</button>}>
      {pending ? <div className="resume-extraction-form"><div className="resume-extraction-notice"><b>识别结果需要手动确认</b><p>当前部署尚未启用外部 PDF 解析接口；文件已安全保留，以下字段沿用现有资料，可直接修改。后续接入解析服务时无需改变数据结构。</p></div>{(["name","school","major","grade","skills","awards"] as const).map((key) => <label key={key}><span>{{ name:"姓名",school:"学校",major:"专业",grade:"年级",skills:"技能",awards:"奖项" }[key]}</span>{key === "skills" || key === "awards" ? <textarea rows={3} value={pending[key]} onChange={(event) => setPending((current) => current ? { ...current, [key]: event.target.value } : current)} /> : <input value={pending[key]} onChange={(event) => setPending((current) => current ? { ...current, [key]: event.target.value } : current)} />}</label>)}</div> : null}
    </SidePanel>
  </>;
}
