...
test("public home renders JA Starlight proposition and media carousel", async()=>{const response=await page("/");assert.equal(response.status,200);const html=await response.text();assert.match(html,/JA 星光计划/);assert.match(html,/让每一次探索/);assert.match(html,/正在发生的成长故事/);assert.match(html,/ja-china-logo\.jpg/);assert.match(html,/实习机会/)});
...
test("JA console is directly accessible during testing", async()=>{const response=await page("/ja-console");assert.equal(response.status,200);const html=await response.text();assert.match(html,/长沙运营后台/);assert.match(html,/JA 测试管理员/);assert.doesNotMatch(html,/此账号没有后台权限/)});
