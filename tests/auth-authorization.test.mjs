...
test("platform authorization uses native sessions and enterprise verification", async () => {
  const [runtime, platform, workspace, admin] = await Promise.all([
    ...
    readFile(new URL("../app/workspace/WorkspaceRoute.tsx", import.meta.url), "utf8"),
    readFile(new URL("../app/ja-console/JAConsoleRoute.tsx", import.meta.url), "utf8"),
  ]);
  ...
  assert.match(workspace, /\/auth\/\$\{route\.role\}/);
